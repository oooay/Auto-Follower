...
{
public:
	BYTETracker(float track_th, float high_th, float match_th, int frame_rate = 30, int lost_time = 100);
	~BYTETracker();

	vector<STrack> update(const vector<Object>& objects, bool reset_flag, int id, int imgWidth, int imgHeight, int resetCenter, float y_th);
	vector<STrack> reset(const vector<Object>& objects);
	Scalar get_color(int idx);
	float iou(vector<int> &atlbrs, vector<int> &btlbrs);
...
private:
	...
	vector<STrack> reset_tracked_stracks;
