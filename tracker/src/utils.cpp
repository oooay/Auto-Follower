...
{
	/////////////////////  ///////////////////////
	map<int, int> exists;// new a map both key and value are int type
	vector<STrack> res;
	for (int i = 0; i < tlista.size(); i++)
	{
		exists.insert(pair<int, int>(tlista[i].track_id, 1));//pair -> combine two num
		res.push_back(tlista[i]);
	}
	for (int i = 0; i < tlistb.size(); i++)
	{
		int tid = tlistb[i].track_id;
		if (!exists[tid] || exists.count(tid) == 0)//key tid has no value or zero value
		{
			exists[tid] = 1;
			res.push_back(tlistb[i]);
		}
...
float BYTETracker::iou(vector<int> &atlbrs, vector<int> &btlbrs)
{
	float iou;
	if (atlbrs.size()*btlbrs.size() == 0)
		return iou;
	...
	vector<float> ious_tmp;
	float box_area = (btlbrs[2] - btlbrs[0] + 1)*(btlbrs[3] - btlbrs[1] + 1);
	float iw = min(atlbrs[2], btlbrs[2]) - max(atlbrs[0], btlbrs[0]) + 1;
	if (iw > 0)
	{
		float ih = min(atlbrs[3], btlbrs[3]) - max(atlbrs[1], btlbrs[1]) + 1;
		if(ih > 0)
		{
			float ua = (atlbrs[2] - atlbrs[0] + 1)*(atlbrs[3] - atlbrs[1] + 1) + box_area - iw * ih;
			iou = iw * ih / ua;
		}
		else
		{
	...
	bool extend_cost, float cost_limit, bool return_cost)
{
	...
	cost_c.assign(cost.begin(), cost.end());//put cost[first,last) to vector cost_c
...
const std::vector<Scalar> COLORS = {
	{46, 242, 203}, {242, 236, 94}, {242, 46, 173}, {58, 131, 242}
};
