...
#include "linefit_ground_segmentation_ros/ground_segmentation.h"
  ...
  void scanCallback(const pcl::PointCloud<pcl::PointXYZ>& cloud) {
    ...
    if (!gravity_aligned_frame_.empty()) {
      ...
      try{
        ...
        Eigen::Affine3d tf;
        tf = tf2::transformToEigen(tf_stamped.transform);
    ...
    for (size_t i = 0; i < cloud.size(); ++i) {
      ...
      if (labels[i] == 1 && cloud[i].z < -0.42) ground_cloud.push_back(cloud[i]);
...
int main(int argc, char** argv) {
  ...
  nh.param("roadside_height", params.roadside_height, params.roadside_height);
